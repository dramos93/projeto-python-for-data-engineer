...
class Gold(PolarsProcessor):
    ...
    def execute(self):
        ...
        df_performance = (
            lazy_df.sort(["code_exchange_rate", "time_last_update_utc"])
            .with_columns(
                [
            ...
            .with_columns(
                [
                    (
                        (pl.col("exchange_rate") - pl.col("prev_rate"))
                        / pl.col("prev_rate")
                        * 100
                    )
                    .round(2)
            ...
            .filter(
                pl.col("prev_rate").is_not_null()
            )  # Remove primeira linha de cada moeda
        )

        # Calcular variações semanais e mensais
        df_performance = df_performance.with_columns(
            [
                pl.col("exchange_rate")
                .shift(7)
                .over("code_exchange_rate")
            ...
            [
                (
                    (pl.col("exchange_rate") - pl.col("rate_7d_ago"))
                    / pl.col("rate_7d_ago")
                    * 100
                )
                .round(2)
                ...
                (
                    (pl.col("exchange_rate") - pl.col("rate_30d_ago"))
                    / pl.col("rate_30d_ago")
                    * 100
                )
                .round(2)
        ...
        last_date = (
            df_performance.select(pl.col("time_last_update_utc").max()).collect().item()
        )
        ...
        # Top 10 Gainers
        top_gainers = (
            df_movers.sort("daily_change_pct", descending=True)
            .head(10)
            .with_row_index("rank")
        ...
        # Top 10 Losers
        top_losers = (
            df_movers.sort("daily_change_pct", descending=False)
            .head(10)
            .with_row_index("rank")
            .with_columns((pl.col("rank") + 1).alias("rank"))
            .select(["rank", "code_exchange_rate", "exchange_rate", "daily_change_pct"])
        ...
        df_volatility = (
            df_performance.group_by("code_exchange_rate")
            .agg(
                [
